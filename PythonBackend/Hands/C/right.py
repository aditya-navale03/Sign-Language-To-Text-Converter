...
def detect_right_hand(landmarks: list) -> bool:
    """
    Detect ASL letter 'C' for right hand (palm facing camera).
    ...
    Returns:
        bool: True if right hand is making ASL 'C' shape, False otherwise
    """
...
if __name__ == "__main__":
    def create_sample_right_c_landmarks():
        """Create sample landmarks that represent a 'C' shape for right hand."""
        landmarks = []
        ...
        landmarks.extend([
            Point(0.45, 0.75, 0.01),  # CMC
            Point(0.4, 0.7, 0.02),    # MCP
            Point(0.35, 0.65, 0.03),  # IP
            Point(0.3, 0.6, 0.04)     # TIP
        ])
        
        # Index finger (4 points: MCP, PIP, DIP, TIP)
        landmarks.extend([
            Point(0.55, 0.65, 0.0),   # MCP
            Point(0.58, 0.5, 0.01),   # PIP
            Point(0.6, 0.4, 0.02),    # DIP
            Point(0.62, 0.32, 0.03)   # TIP
        ])
        
        # Middle finger (4 points: MCP, PIP, DIP, TIP)
        landmarks.extend([
            Point(0.5, 0.62, 0.0),    # MCP
            Point(0.52, 0.45, 0.01),  # PIP
            Point(0.53, 0.35, 0.02),  # DIP
            Point(0.54, 0.25, 0.03)   # TIP
        ])
        
        # Ring finger (4 points: MCP, PIP, DIP, TIP)
        landmarks.extend([
            Point(0.45, 0.63, 0.0),   # MCP
            Point(0.46, 0.48, 0.01),  # PIP
            Point(0.47, 0.38, 0.02),  # DIP
            Point(0.48, 0.3, 0.03)    # TIP
        ])
        
        # Pinky finger (4 points: MCP, PIP, DIP, TIP)
        landmarks.extend([
            Point(0.4, 0.65, 0.0),    # MCP
            Point(0.39, 0.52, 0.01),  # PIP
            Point(0.38, 0.42, 0.02),  # DIP
            Point(0.37, 0.35, 0.03)   # TIP
        ])
        
        return landmarks
    
    # Test the right hand detector
    sample_landmarks = create_sample_right_c_landmarks()
    result = detect_right_hand_c(sample_landmarks)
    
    print("Right Hand ASL 'C' Detector Test:")
    print(f"Sample landmarks detected as 'C': {result}")
    
    # Test with invalid input
    print(f"Empty landmarks: {detect_right_hand_c([])}")
    print(f"None input: {detect_right_hand_c(None)}")